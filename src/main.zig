...
const corpus_from_pgn = @import("tools/corpus_from_pgn.zig");
const corpus_search_label = @import("tools/corpus_search_label.zig");
const corpus_validate = @import("tools/corpus_validate.zig");
const datagen = @import("tools/datagen.zig");
const fen = @import("core/fen.zig");
const history_report = @import("tools/history_report.zig");
const legal = @import("movegen/legal.zig");
const make_unmake = @import("movegen/make_unmake.zig");
const move = @import("core/move.zig");
const move_diag = @import("tools/move_diag.zig");
const nnue_dump = @import("tools/nnue_dump.zig");
...
const ft6_stats = @import("tools/ft6_stats.zig");
const time_profile = @import("tools/time_profile.zig");
...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, mode, "ft6_stats")) {
        const depth = if (args.next()) |t| try std.fmt.parseInt(u8, t, 10) else 3;
        try ft6_stats.run(stdout, depth);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "corpus_from_pgn")) {
        ...
        try corpus_from_pgn.run(stdout, path, min_ply, stride);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "corpus_validate")) {
        ...
        try corpus_validate.run(stdout, path, validation_permille, test_permille, seed, split_mode);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "corpus_search_label")) {
        ...
        const emit_header = if (args.next()) |text|
            ...
            true;
        try corpus_search_label.run(stdout, path, depth, hash_mb, eval_options, emit_header);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "corpus_search_label_nodes")) {
        ...
        const emit_header = if (args.next()) |text|
            ...
            true;
        try corpus_search_label.runNodes(stdout, path, node_limit, hash_mb, eval_options, emit_header);
        try stdout.flush();
        return;
    }

    if (std.mem.eql(u8, mode, "datagen")) {
        const seed_text = args.next() orelse return error.InvalidCommand;
    ...
    if (std.mem.eql(u8, mode, "nnue_dump")) {
        ...
        try nnue_dump.run(stdout, net_path, fen_file);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "root_trace")) {
        ...
        const pos = try parsePositionArg(args.next());
        try root_trace.run(stdout, &pos, depth);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "search_profile")) {
        ...
        const pos = try parsePositionArg(args.next());
        const eval_options = try parseEvalOptions(args.next(), args.next());
        try search_profile.runWithOptions(stdout, &pos, depth, eval_options);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "raw_search_probe")) {
        ...
        const pos = try parsePositionArg(args.next());
        const eval_options = try parseEvalOptions(args.next(), args.next());
        try raw_search_probe.runWithOptions(stdout, &pos, depth, eval_options);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "search_profile_sequence")) {
        ...
        const eval_options = try parseEvalOptions(args.next(), args.next());
        try search_profile_sequence.runFileWithOptions(stdout, fen_file, depth, eval_options);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "time_profile")) {
        ...
        const pos = try parsePositionArg(args.next());
        const eval_options = try parseEvalOptions(args.next(), args.next());
        try time_profile.runWithOptions(stdout, &pos, movetime_ms, eval_options);
        try stdout.flush();
        return;
    }
    ...
    if (std.mem.eql(u8, mode, "time_profile_tc")) {
        ...
        const pos = try parsePositionArg(args.next());
        const eval_options = try parseEvalOptions(args.next(), args.next());
        try time_profile.runSuddenDeathWithOptions(stdout, &pos, wtime_ms, btime_ms, winc_ms, binc_ms, eval_options);
        try stdout.flush();
        return;
    }
...
test "parse eval options defaults to the builtin net at the default scale" {
    ...
    // Reference the single-source constant, not a literal — the default is per-net
    // tuned and changes across releases (44 -> 54 -> 62).
